...
public class Qes6 {
	...
	public static void main(String[] args) {
		...
		// 配列[values]の長さ分ループ処理。値は変数[app]に代入
		for(String app: values) {
			// [app]の値で条件指定
			switch(app) {
				// [app]の値が"パソコン"なら下記の処理を実行
				case "パソコン":
					// 0~11の値をランダムで取得し再代入
				...
				// [app]の値が"テレビかディスプレイ"なら下記の処理を実行
				case "テレビ":
				case "ディスプレイ":
					// テレビとディスプレイの値変わってしまうため、[index]の再代入は行わない
					// [app]の値がテレビなら[index]値、ディスプレイなら11から[index]値を引いた数を変数[result]へ代入
